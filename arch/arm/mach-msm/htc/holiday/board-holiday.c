...
static uint32_t mhl_reset_pin_ouput_table[] = {
	GPIO_CFG(HOLIDAY_GPIO_MHL_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.get_adc_cb	= holiday_get_usbid_adc,
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	.gpio_intr = HOLIDAY_GPIO_MHL_INT,
	.gpio_reset = HOLIDAY_GPIO_MHL_RESET,
	.ci2ca = 0,
...
static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(HOLIDAY_GENERAL_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
...
static struct i2c_board_info msm_i2c_gsbi5_info[] = {
	{
		...
		.platform_data = &holiday_ts_atmel_data,
...
static struct platform_device rpm_regulator_early_device __devinitdata = {
	...
	.id	= 0,
	.dev	= {
		.platform_data = &holiday_rpm_regulator_early_pdata,
	},
};
...
static struct platform_device rpm_regulator_device __devinitdata = {
	...
	.id	= 1,
	.dev	= {
		.platform_data = &holiday_rpm_regulator_pdata,
	},
};
...
static void __init holiday_early_memory(void)
{
...
static uint32_t msm_spi_gpio[] = {
	...
	GPIO_CFG(HOLIDAY_SPI_DI,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(HOLIDAY_SPI_CS,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(HOLIDAY_SPI_CLK, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static void __init holiday_init(void)
{
	...
	platform_add_devices(holiday_devices,
			     ARRAY_SIZE(holiday_devices));

        holiday_gpio_mpp_init();
	...
	if (properties_kobj) {
          rc = sysfs_create_group(properties_kobj, &holiday_properties_attr_group);
	}
...
static void __init holiday_fixup(struct tag *tags,
...
MACHINE_START(HOLIDAY, "holiday")	
	...
	.init_very_early = holiday_early_memory,
	.restart = msm_restart,
MACHINE_END
