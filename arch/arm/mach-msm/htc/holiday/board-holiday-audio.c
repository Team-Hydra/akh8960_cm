...
void holiday_snddev_poweramp_on(int en)
{
	...
	if (en) {
		msleep(50);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HOLIDAY_HAP_ENABLE), 1);
		set_speaker_amp(1);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_SPEAKER;
	} else {
		set_speaker_amp(0);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HOLIDAY_HAP_ENABLE), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_SPEAKER;
	}
}
...
void holiday_snddev_hsed_pamp_on(int en)
{
	...
	if (en) {
		msleep(50);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_HP_EN), 1);
		set_headset_amp(1);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_HEADSET;
	} else {
		set_headset_amp(0);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_HP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_HEADSET;
	}
}
...
void holiday_snddev_receiver_pamp_on(int en)
{
	...
	if (en) {
		if (!atomic_read(&aic3254_ctl))
			gpio_set_value(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_HP_EN), 1);
			set_handset_amp(1);
			curr_rx_mode |= BIT_RECEIVER;
	} else {
		if (!atomic_read(&aic3254_ctl))
			set_handset_amp(0);
			gpio_set_value(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_HP_EN), 0);
			curr_rx_mode &= ~BIT_RECEIVER;
	}
}
...
void holiday_snddev_bmic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_MIC_SEL), 0);
	} else {
...
void holiday_snddev_emic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_MIC_SEL), 1);
	}
}
...
void holiday_snddev_stereo_mic_pamp_on(int en)
{
	...
	if (en) {
		...
		/* select external mic path */
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_MIC_SEL), 0);
	} else {
		ret = pm8058_micbias_enable(OTHC_MICBIAS_0, OTHC_SIGNAL_OFF);
		if (ret)
...
void holiday_voltage_on(int en)
{
}
...
void holiday_audio_gpios_init(void)
{
	pr_info("%s\n", __func__);
	gpio_request(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_HP_EN), "AUD_SPK_ENO");
	gpio_request(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_MIC_SEL), "AUD_HANDSET_ENO");
}
