...
/* function prototype */
void holiday_snddev_bmic_pamp_on(int);
...
static uint32_t msm_codec_reset_gpio[] = {
	...
	/* Timpani Reset */
	GPIO_CFG(HOLIDAY_AUD_QTR_RESET, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
};
...
void holiday_snddev_tx_pamp_on(int en)
{
	int rc = 0;
	int call_state = 0;
...
void holiday_snddev_emic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(HOLIDAY_AUD_MIC_SEL), 1);
	} else {
...
void holiday_usb_headset_on(int en)
{
	int rc;
	pr_info("%s\n", __func__);
...
static struct q6v2audio_analog_ops ops = {
	...
	.usb_headset_enable     = holiday_usb_headset_on,
};
...
void __init holiday_audio_init(void)
{
	...
	gpio_set_value(HOLIDAY_AUD_QTR_RESET, 0);
	mdelay(1);
	gpio_set_value(HOLIDAY_AUD_QTR_RESET, 1);
