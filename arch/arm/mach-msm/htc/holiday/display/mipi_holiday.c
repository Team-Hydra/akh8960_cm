...
static int mipi_holiday_send_cmd(struct dsi_cmd_desc *cmd, unsigned int len, bool clk_ctrl)
{
	int ret = 0;
...
static struct msm_panel_common_pdata *mipi_holiday_pdata;

static int mipi_holiday_lcd_init(void);
...
void mipi_holiday_panel_type_detect(void)
{
...
static int mipi_holiday_lcd_on(struct platform_device *pdev)
{
	...
	mipi  = &mfd->panel_info.mipi;
        ...
        mipi_holiday_send_cmd(mipi_power_on_cmd, mipi_power_on_cmd_size, false);
...
static int mipi_holiday_display_on(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	unsigned int val = 0;
	...
	mipi_holiday_send_cmd(novatek_display_on_cmds, ARRAY_SIZE(novatek_display_on_cmds), true);
...
static int mipi_holiday_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static int mipi_holiday_display_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;

	mfd = platform_get_drvdata(pdev);

	mipi_holiday_send_cmd(mipi_power_off_cmd, mipi_power_off_cmd_size, true);

	return 0;
}

static unsigned char holiday_shrink_pwm(int br)
{
...
inline void mipi_dsi_set_backlight(struct msm_fb_data_type *mfd, int level)
{
	...
	led_pwm1[1] = holiday_shrink_pwm(mfd->bl_level);

	if (mfd->bl_level == 0 || board_mfg_mode() == 4) {
		led_pwm1[1] = 0;
	}

        mipi_holiday_send_cmd(novatek_cmd_backlight_cmds, ARRAY_SIZE(novatek_cmd_backlight_cmds), (level > 0));

	bl_level_prevset = mfd->bl_level;
}

static void mipi_holiday_set_backlight(struct msm_fb_data_type *mfd)
{
  
        mipi_dsi_set_backlight(mfd, mfd->bl_level);
}

static int mipi_holiday_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_holiday_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static struct msm_fb_panel_data holiday_panel_data = {
	.on		= mipi_holiday_lcd_on,
        .off		= mipi_holiday_lcd_off,
        .set_backlight  = mipi_holiday_set_backlight,
	.late_init	= mipi_holiday_display_on,
	.early_off	= mipi_holiday_display_off,
};

static int ch_used[3];

int mipi_holiday_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	ret = mipi_holiday_lcd_init();
	if (ret) {
		pr_err("mipi_holiday_lcd_init() failed with ret %u\n", ret);
		return ret;
	}

	pdev = platform_device_alloc("mipi_holiday", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;
...
static int mipi_holiday_lcd_init(void)
{
        mipi_holiday_panel_type_detect();
